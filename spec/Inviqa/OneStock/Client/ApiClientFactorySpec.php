...
{
    function it_creates_a_http_client_when_not_in_test_mode(Config $config, LoggerInterface $logger)
    {
        $config->endpoint()->willReturn('foo');
        $config->username()->willReturn('bar');
        $config->password()->willReturn('pass');

        $this->createApiClient($config, $logger)->shouldHaveType(ApiClient::class);
    }
}
